                ...
                <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
                        ...
                        <form onSubmit={handleCreateTask} className="p-6">
                            ...
                            <div className="mb-8"><label className="block text-sm font-bold text-slate-700 mb-2">Assignee</label><select value={taskForm.assignedTo} onChange={(e) => setTaskForm({ ...taskForm, assignedTo: e.target.value })} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none font-bold text-slate-700 text-sm"><option value="">Unassigned</option><option value={user?._id}>Me ({user?.name})</option>{project.team?.filter(m => m._id !== user?._id).map(m => (<option key={m._id} value={m._id}>{m.name}</option>))}</select></div>
